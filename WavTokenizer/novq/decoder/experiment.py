    ...
    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        ...
        if optimizer_idx == 1:
            ...
            if not self.feature_extractor.novq:
                embed_onehot_sum = quantizer._codebook.embed_onehot_sum
                used_codes = (embed_onehot_sum > 0).sum().item()
                vocab_size = quantizer._codebook.codebook_size
                total_codes = embed_onehot_sum.sum().item()
                utilization = used_codes / vocab_size if vocab_size > 0 else 0.0
                entropy = -np.sum((embed_onehot_sum.cpu().numpy() / total_codes) * np.log(embed_onehot_sum.cpu().numpy() / total_codes + 1e-10))
                self.log("quantizer/used_codes", used_codes)
                self.log("quantizer/utilization", utilization)
                self.log("quantizer/entropy", entropy)
                self.log("quantizer/commit_loss", commit_loss)

                if not getattr(self.feature_extractor, 'freeze_encoder_vq', False):
            ...
            # ── Every-step latent scalar diagnostics (live batch, all frames) ──────────
            with torch.no_grad():
                _emb = self.feature_extractor.encodec.encoder(audio_input.unsqueeze(1))  # (B, D, T)
            _Z = _emb.permute(0, 2, 1).reshape(-1, _emb.shape[1]).float()  # (B*T, D) — all frames
            _Z_c = _Z - _Z.mean(0)         # center across frames
            _sv = torch.linalg.svdvals(_Z_c)  # (min(B*T, D),) — up to 512 singular values
            _p = _sv / (_sv.sum() + 1e-10)
            self.log("latent/frobenius_norm",         torch.linalg.norm(_Z_c, 'fro').item(), on_step=True)
            self.log("latent/spectral_norm",          _sv[0].item(),                          on_step=True)
            self.log("latent/nuclear_norm",           _sv.sum().item(),                        on_step=True)
            self.log("latent/effective_rank_entropy", torch.exp(-(_p * torch.log(_p + 1e-10)).sum()).item(), on_step=True)
            _l2 = _Z.norm(dim=1).cpu().numpy()
            self.log("latent/l2_norm_mean", float(_l2.mean()), on_step=True)
            self.log("latent/l2_norm_std",  float(_l2.std()),  on_step=True)
            ...
            if self.global_step % self.plot_every_n_steps == 0 and self.global_rank == 0:
                ...
                # Reuse _Z computed above (no second encoder pass needed)
                features = _Z.cpu().numpy()
