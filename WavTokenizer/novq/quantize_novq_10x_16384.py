"""
Codebook Initialization for NoVQ WavTokenizer
...
3. Runs sklearn KMeans++ (D² seeding + Lloyd's) to initialize the codebook
4. Saves the model with the sklearn codebook
"""
...
LATENT_DIM     = 512
CODEBOOK_SIZE  = 4096 * 4
N_AUDIO_FILES  = 500      # number of training files to sample
# N_LATENTS_FOR_KMEANS = CODEBOOK_SIZE * 4   # number of latents to sample for k-means
N_LATENTS_FOR_KMEANS = CODEBOOK_SIZE * 10  # for testing
CLUSTER_SIZE_TO_SAVE = 0.25
...
# =============================================================================
# 6. sklearn KMeans++ (D² seeding + Lloyd's)
# =============================================================================

from sklearn.cluster import KMeans

KMEANS_ITERS_SKLEARN = 300

data_np = latents.detach().cpu().numpy()

print(f'Running sklearn KMeans++ (k={CODEBOOK_SIZE}, max_iter={KMEANS_ITERS_SKLEARN})...')
km = KMeans(
    n_clusters=CODEBOOK_SIZE,
    init='k-means++',
    n_init=1,
    max_iter=KMEANS_ITERS_SKLEARN,
    random_state=42,
    verbose=1,
)
km.fit(data_np)

centroids_sklearn = torch.tensor(km.cluster_centers_, dtype=torch.float32)
distortion_sklearn = compute_distortion(torch.tensor(data_np, dtype=torch.float32), centroids_sklearn)

print(f'\nsklearn KMeans++: distortion = {distortion_sklearn:.6f}')
print(f'  sklearn inertia (sum): {km.inertia_:.4f}')
print(f'  sklearn inertia/N:     {km.inertia_ / len(data_np):.6f}')
print(f'  Iterations:            {km.n_iter_}')

plot_pca_with_centers(
    centroids_sklearn.cpu(),
    'sklearn KMeans++',
    save_name=f'pca_sklearnpp_{N_LATENTS_FOR_KMEANS}.png',
)
plot_codebook_usage(
    centroids_sklearn.cpu(),
    'sklearn KMeans++',
    save_name=f'usage_sklearnpp_{N_LATENTS_FOR_KMEANS}.png',
)
...
def build_cluster_size(centroids: torch.Tensor, X: torch.Tensor, chunk: int = 4096) -> torch.Tensor:
    """Compute cluster sizes (number of training points assigned to each centroid)."""
    k = centroids.shape[0]
    counts = torch.zeros(k, dtype=torch.float32)
    for s in range(0, len(X), chunk):
        xc = X[s:s + chunk].float()
        d2 = (
            xc.pow(2).sum(1, keepdim=True)
            - 2.0 * xc @ centroids.t()
            + centroids.pow(2).sum(1, keepdim=True).t()
        ).clamp(min=0.0)
        ids = d2.argmin(dim=1)
        counts.scatter_add_(0, ids, torch.ones(len(xc)))
    return counts
...
out_path = os.path.join(SAVE_DIR, f'ep{BEST_EPOCH}_kmeans_sklearnpp_{N_LATENTS_FOR_KMEANS}_thresh{CLUSTER_SIZE_TO_SAVE}.ckpt')
print('Saving sklearnpp...')
save_model_with_codebook(CKPT_PATH, out_path, centroids_sklearn.to("cpu"), latents.to("cpu"))
...
counts = _assign_counts(centroids_sklearn.cpu(), latents.cpu())
used   = int((counts > 0).sum())
...
comparison = {
    ...
    'methods': {
        'sklearnpp': {'distortion': distortion_sklearn, 'utilization': util, 'entropy_bits': ent}
    }
}
...
print()
print(f'Method          : sklearn KMeans++')
print(f'Distortion (MSE): {distortion_sklearn:.6f}')
