...
def main():
    ...
    for p in tqdm(files):
        ...
        with torch.no_grad():
            L = min(wav.shape[-1], audio_out.shape[-1])
            ml = mel_loss_fn(audio_out[..., :L], wav[..., :L])
    ...
    if all_codes:
        ...
        report = {
            ...
            "mel_loss_mean": float(np.mean(mel_losses)) if mel_losses else None,
            "mel_loss_std": float(np.std(mel_losses)) if mel_losses else None,
