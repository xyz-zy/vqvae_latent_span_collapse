    ...
    def training_step(self, batch, batch_idx, optimizer_idx, **kwargs):
        ...
        if optimizer_idx == 1:
            ...
            if self.global_step % self.plot_every_n_steps == 0 and self.global_rank == 0:
                ...
                codebook_svd_solver = "full" if codebook.shape[0] <= 2**14 else "randomized"
                latent_eff_dim = pca_effective_dim(features, var_thresh=0.99, svd_solver="randomized")
                codebook_eff_dim = pca_effective_dim(codebook, var_thresh=0.99, svd_solver=codebook_svd_solver)
